...
export function genereatePrivateKeyFromSeed(seed: string) {
  const keypair = Keypair.fromSeed(sha256(seed))
  return {
...
export function generatePrivateKeyFromAddresses(addresses: string[]) {
  const seed = addresses.join("-")
  return genereatePrivateKeyFromSeed(seed)
}
