  ...
  try {
    const result = await sql`
      INSERT INTO positions (
        ...
        add_liquidity_max_attempts, remove_liquidity_max_attempts, public_key
      ) VALUES (
        ...
        ${addLiquidityMaxAttempts}, ${removeLiquidityMaxAttempts}, ${publicKey}
      ) RETURNING id;
    `
...
export async function getUserPositions(userAddress: string) {
  try {
    const result =
      await sql`SELECT * FROM positions WHERE user_address = ${userAddress};`
    return result.rows
  } catch (error) {
    console.error("Failed to fetch positions:", error)
    throw error
